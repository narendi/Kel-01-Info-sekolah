...
const Jurusan = () => {
  ...
  const fetchDataImage = async () => {
    try {
      const response = await axios.get("http://localhost:3100/image/all");
      setDataImage(response.data);
    } catch (error) {
      toast.error("Terjadi kesalahan saat menampilkan data Item");
    }
  };
  ...
  const updateImage = async () => {
    try {
      ...
      await axios.patch(`http://localhost:3100/image/${id}`, formData);
      fetchDataImage();
  ...
  const deleteImage = async () => {
    try {
      setLoading(true);
      await axios.delete(`http://localhost:3100/image/${id}`);
      fetchDataImage();
  ...
  return (
    <div className="container mx-auto">
      <div className="justify-start pt-6 pl-9 pb-6 flex pr-7">
        <div className="font-bold font-serif pr-5 text-xl">Slider</div>
        <div>
          <button
          ...
          <div className="fixed z-10 inset-0 overflow-y-auto">
            <div className="flex items-center justify-center min-h-screen">
              <div className="bg-white w-1/2 p-6 rounded-lg">
                ...
                <h1 className="text-2xl font-bold mb-4">Tambah Item Baru</h1>
